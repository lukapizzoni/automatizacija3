...
const { generirajDokumentPdf } = require("./generiraj-dokument-pdf");
...
async function obdelajRacun({ besedilo, ime_datoteke, vlozitelj, pdf_base64, vir }) {
  ...
  // --- 3b. Generiraj "lep" standardiziran PDF izpis (za lazji pregled/arhiv --
  //         NI nadomestilo za original in NI oddaja na FURS) ---
  let napaka_generiranja_pdf_besedilo = null; // ZACASNO -- za diagnostiko
  try {
    const generiranBuffer = await generirajDokumentPdf(izlusceno, {
      dokument_id: dokument.id,
      generirano_datum: new Date(dokument.ustvarjeno || Date.now()).toLocaleString("sl-SI"),
    });
    const generiranaPot = `generirano_${dokument.id}.pdf`;
    const { error: napakaGeneriranega } = await supabase.storage
      .from("racuni")
      .upload(generiranaPot, Buffer.from(generiranBuffer), { contentType: "application/pdf", upsert: true });

    if (!napakaGeneriranega) {
      const { data: javniLinkGeneriran } = supabase.storage.from("racuni").getPublicUrl(generiranaPot);
      if (javniLinkGeneriran?.publicUrl) {
        await supabase.from("dokumenti").update({ generiran_pdf_url: javniLinkGeneriran.publicUrl }).eq("id", dokument.id);
        dokument.generiran_pdf_url = javniLinkGeneriran.publicUrl;
      }
    } else {
      console.error("Napaka pri nalaganju generiranega PDF-ja:", napakaGeneriranega);
      napaka_generiranja_pdf_besedilo = "Nalaganje: " + String(napakaGeneriranega.message || napakaGeneriranega);
    }
  } catch (napakaGeneriranja) {
    // Generiranje "lepega" PDF-ja ni kriticno -- ce spodleti, glavni podatki so ze shranjeni
    console.error("Napaka pri generiranju PDF izpisa:", napakaGeneriranja);
    napaka_generiranja_pdf_besedilo = "Generiranje: " + String(napakaGeneriranja.message || napakaGeneriranja);
  }
  ...
  return { uspesno: true, dokument, napaka_nalaganja_pdf_besedilo, napaka_generiranja_pdf_besedilo };
}
